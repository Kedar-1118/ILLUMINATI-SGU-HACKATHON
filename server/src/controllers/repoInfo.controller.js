...
const GITHUB_TOKEN = process.env.GITHUB_TOKEN; // replace this with your GitHub token
...
export default async function getRepoInfo(req, res) {
    ...
    try {
        ...
        const responses = await Promise.all(
            urls.map(url =>
                axiosInstance.get(url).catch(err => {
                    console.error(`Error fetching ${url}:`, err.response?.status, err.response?.data?.message || err.message);
                    return { data: [] }; // fallback to avoid crashing
                })
            )
        );

        const [repoData, contributors, languages, issues, pulls, branches, tags, commits] = responses;
        ...
        res.status(500).json({
            error: 'Failed to fetch repository data. Ensure the repository format is correct (e.g., owner/repo).'
        });
    };
}
